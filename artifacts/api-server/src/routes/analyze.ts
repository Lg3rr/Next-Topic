...
function generateAdvancedInsights(sessions: Session[], aiSummary?: any) {
  console.log("[INSIGHTS] generateAdvancedInsights called with", sessions.length, "sessions");
  const patterns = [];
  const recommendations = [];
  
  if (!sessions || !Array.isArray(sessions) || sessions.length === 0) {
    console.log("[INSIGHTS] No sessions, returning empty insights");
  ...
  const subjectPerf = Object.entries(subjectStats).map(([s, d]) => ({
    subject: s,
  ...
  // Skipped days detection
  const dateRange = sessions.length > 10;
  if (dateRange) {
  ...
  console.log("[INSIGHTS] Generated", patterns.length, "patterns and", recommendations.length, "recommendations");
  return {
    patterns,
    recommendations,
    timestamp: Date.now(),
...
router.post("/analyze", async (req, res): Promise<void> => {
  try {
    console.log("[BACKEND] POST /analyze called");
    console.log("[BACKEND] Request body:", req.body);
    
    const { sessions, allSessions, interviewContext } = req.body;
    
    console.log("[BACKEND] Extracted sessions:", sessions ? sessions.length : "undefined", "items");
    console.log("[BACKEND] Extracted allSessions:", allSessions ? allSessions.length : "undefined", "items");
    console.log("[BACKEND] Extracted interviewContext:", interviewContext);

    if (!Array.isArray(sessions)) {
      console.error("[BACKEND] ❌ sessions is not an array");
      res.status(400).json({ error: "sessions must be an array" });
      return;
    }
    
    console.log("[BACKEND] ✅ sessions validation passed");

    const apiKeys = getApiKeys();
    console.log("[BACKEND] API keys available:", apiKeys.length);
    
    if (apiKeys.length === 0) {
      console.error("[BACKEND] ❌ No API keys configured");
      res.status(500).json({ error: "No Gemini API key configured" });
      return;
    }
    
    console.log("[BACKEND] ✅ API keys check passed");
...
${interviewContext}` : ""}`;
    ...
    if (!jsonMatch) {
      console.error("[BACKEND] ❌ Failed to extract JSON from Gemini response");
      console.error("[BACKEND] Raw response:", raw);
      res.status(500).json({ error: "Failed to parse analysis response. Please try again." });
      return;
    }

    console.log("[BACKEND] ✅ JSON extracted from Gemini response");
    let analysis;
...
try {
  ...
  return res.status(500).json({ error: "Gemini returned invalid JSON." });
    ...
    console.log("[BACKEND] Parsed analysis:", analysis);
    
    // Generate advanced insights from full session history
    console.log("[BACKEND] About to generate insights...");
    const insightsData = Array.isArray(allSessions) ? allSessions : sessions;
    console.log("[BACKEND] Using", insightsData.length, "sessions for insights generation");
    
    const insights = generateAdvancedInsights(insightsData);
    console.log("[BACKEND] ✅ Insights generated:", insights);
    ...
    console.error("[BACKEND] ❌ EXCEPTION IN ANALYZE ROUTE");
    console.error("[BACKEND] Error type:", typeof err);
    console.error("[BACKEND] Error instanceof Error:", err instanceof Error);
    ...
    console.error("[BACKEND] Sending error response:", msg);
